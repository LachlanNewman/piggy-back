...
**Reason**: Replaced by `ProfileCompletionForm` (see `profile-completion-gate` spec). The new form collects only the 3 biometric fields; name and email are sourced from IdP claims. The form is gated behind post-login profile status check rather than rendered unconditionally.
**Migration**: Delete `SignupForm.jsx`; implement `ProfileCompletionForm.jsx` per the `profile-completion-gate` spec.
